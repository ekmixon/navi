...
def exclusions():
    try:
        for exclusion in tio.exclusions.list():
            ...
            click.echo("-" * 150)
            click.echo(f'{str(exclusion["members"])}')
        click.echo()
    except AttributeError:
        click.echo("\nCheck your permissions or your API keys\n")
...
def assets(tag):
    if tag:
        data = db_query(
            f"select ip_address, fqdn, aes, acr from assets LEFT JOIN tags ON uuid == asset_uuid where tag_uuid=='{tag}';"
        )
        ...
        try:
            ...
            for asset in tio.workbenches.assets():
                ...
                sources = "".join(f", {str(source['name'])}" for source in asset["sources"])
...
def agroups():
    ...
    try:
        ...
        for group in tio.access_groups.list():
            ...
            click.echo(
                "{:25s} {:40s} {:25} {:60s}".format(
                    textwrap.shorten(str(group['name']), width=25),
                    str(group['id']),
                    updated,
...
def status():
    try:
        ...
        click.echo("-" * 25)
        click.echo(f'Container ID :  {session_data["container_id"]}')
        click.echo(f'Container UUID : {session_data["container_uuid"]}')
        click.echo(f'Container Name :  {session_data["container_name"]}')
        click.echo(f'Site ID : {data["analytics"]["site_id"]}')
        click.echo(f'Region :  {data["region"]}')

        click.echo("\nLicense information")
        click.echo("-" * 25)
        click.echo(f"Licensed Assets :  {get_licensed()}")
        click.echo(f"""Agents Used :  {data['license']["agents"]}""")
        try:
            click.echo(f"""Expiration Date :  {data['license']["expiration_date"]}""")
        except KeyError:
            pass
        click.echo(f"""Scanners Used :  {data['license']["scanners"]}""")
        click.echo(f'Users :  {data["license"]["users"]}')
        ...
        try:
            for key in data["license"]["apps"]:
                ...
                try:
                    click.echo(
                        f'Expiration:  {str(data["license"]["apps"][key]["expiration_date"])}'
                    )

                except KeyError:
                    pass
                click.echo(f'Mode:  {str(data["license"]["apps"][key]["mode"])}')
                click.echo()
        except KeyError:
            pass
        ...
        for agent in tio.agents.list():
            ...
            try:
                for group in agent['groups']:
                    groups = groups + f", {group['name']}({group['id']})"
            except KeyError:
                pass
            ...
            if uuid:
                click.echo(
                    "{:30s} {:20s} {:20s} {:20s} {:6s} {}".format(
                        textwrap.shorten(str(agent['name']), width=30),
                        str(agent['ip']),
                        last_connect_time,
                        last_scanned_time,
                        str(agent['status']),
    ...
    try:
        ...
        for tag_values in tio.tags.list():
            ...
            click.echo(
                "{:55s} : {:55s} {}".format(
                    textwrap.shorten(
                        str(tag_values['category_name']), width=55
                    ),
                    textwrap.shorten(tag_value, width=55),
                    uuid,
                )
            )
    ...
    try:
        ...
        with conn:
            ...
            for settings in data:
                click.echo(f"\nYour email server: {settings[0]}")
                click.echo(f"The email port is: {settings[1]}")
                click.echo(f"Your email is: {settings[2]}\n")
    except Error:
...
def version():
    click.echo(f"\nCurrent Navi Version: {navi_version()}\n")
...
@click.argument('uid')
def auth(uid):
    info = request_data("GET", f"/users/{uid}/authorizations")
...
def templates(policy, scan):
    ...
    if template_type:
        try:
            ...
            for policy in tio.editor.template_list(template_type):
...
def audits(name, uuid):
    ...
    if name and uuid:
        data = db_query(
            f"SELECT assets.fqdn, compliance.check_name, compliance.status FROM compliance LEFT OUTER JOIN assets ON assets.uuid = compliance.asset_uuid where compliance.audit_file='{name}' and compliance.asset_uuid='{uuid}';"
        )
        ...
        data = db_query(
            f"SELECT assets.fqdn, compliance.check_name, compliance.status FROM compliance LEFT OUTER JOIN assets ON assets.uuid = compliance.asset_uuid where audit_file='{name}';"
        )
        ...
        data = db_query(
            f"SELECT assets.fqdn, compliance.check_name, compliance.status FROM compliance LEFT OUTER JOIN assets ON assets.uuid = compliance.asset_uuid where asset_uuid='{uuid}';"
        )
